...
my_error_joint_rank_col_PL2 = rep(NA,n)
my_error_joint_rank_row_PL2 = rep(NA,n)

ajive_error_joint_rank_c = rep(NA,n)
ajive_error_joint_rank_r = rep(NA,n)

my_error_total_rank1_ED = rep(0,n)
my_error_total_rank2_ED = rep(0,n)
...
for (i in 1:n){
  ...
  my_error_joint_rank_col_PL2[i] = output_ajive[[i]]$my_error_joint_rank_col_PL
  my_error_joint_rank_row_PL2[i] = output_ajive[[i]]$my_error_joint_rank_row_PL
...
pdf(file = paste(fig.path,"Comparison on Total Rank Estimation_TCGA_Setting.pdf",sep=""), width = 5, height = 4)
print(gg_total)
dev.off()
