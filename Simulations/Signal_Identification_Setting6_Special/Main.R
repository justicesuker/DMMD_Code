...
# Total rank 20, 18
# Joint column either 0 or 18
# Joint row rank either 0 or 18
...
output <- foreach (i = 1:nrep) %dopar% {
  ...
  # Record the absolute difference of row decomposition in DMMD
  # Use absolute error because either joint/individual structure is 0. 0 cannot be the denominator.  
  ...
  # Record the absolute difference of column decomposition in DMMD
  ...
  jive_row_joint_error1 = Fnorm(jive_result_row$joint[[1]] - J1_r)^2
  jive_row_joint_error2 = Fnorm(jive_result_row$joint[[2]] - J2_r)^2
  jive_row_ind_error1 = Fnorm(jive_result_row$individual[[1]] - I1_r)^2
  jive_row_ind_error2 = Fnorm(jive_result_row$individual[[2]] - I2_r)^2
  
  # Record the errors of column decomposition in JIVE
  jive_col_joint_error1 = Fnorm(t(jive_result_col$joint[[1]]) - J1_c)^2
  jive_col_joint_error2 = Fnorm(t(jive_result_col$joint[[2]]) - J2_c)^2
  jive_col_ind_error1 = Fnorm(t(jive_result_col$individual[[1]]) - I1_c)^2
  jive_col_ind_error2 = Fnorm(t(jive_result_col$individual[[2]]) - I2_c)^2
...
stopCluster(cl)
save(output, file = "Simulations/Signal_Identification_Setting6_Special/output.RData")