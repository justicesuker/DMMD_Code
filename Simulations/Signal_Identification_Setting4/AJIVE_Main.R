...
output <- foreach (i = 1:nrep, .errorhandling = 'pass') %dopar% {
  ...
  ajive_result_row <- ajive(data_row, initial_signal_ranks, joint_rank = 3)
  
  # Run JIVE on the column direction - column-wise decomposition
  ajive_result_col <- ajive(data_col, initial_signal_ranks, joint_rank = 4)
  ...
  ajive_row_joint_error1 = Fnorm(t(ajive_J1r) - J1_r)^2/Fnorm(J1_r)^2
  ajive_row_joint_error2 = Fnorm(t(ajive_J2r) - J2_r)^2/Fnorm(J2_r)^2
  ajive_row_ind_error1 = Fnorm(t(ajive_I1r) - I1_r)^2/Fnorm(I1_r)^2
  ajive_row_ind_error2 = Fnorm(t(ajive_I2r) - I2_r)^2/Fnorm(I2_r)^2
  
  # Record the errors of column decomposition in JIVE
  ajive_col_joint_error1 = Fnorm(ajive_J1c - J1_c)^2/Fnorm(J1_c)^2
  ajive_col_joint_error2 = Fnorm(ajive_J2c - J2_c)^2/Fnorm(J2_c)^2
  ajive_col_ind_error1 = Fnorm(ajive_I1c - I1_c)^2/Fnorm(I1_c)^2
  ajive_col_ind_error2 = Fnorm(ajive_I2c - I2_c)^2/Fnorm(I2_c)^2
...
stopCluster(cl)
save(output, file = "Simulations/Signal_Identification_Setting4/AJIVE_output.RData")
