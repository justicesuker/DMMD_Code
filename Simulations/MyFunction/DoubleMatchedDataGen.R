...
# Generate double matched data that are convinient to compare the size of signal matrix and noise matrix
# Refer to the simulation part (section 3.1) in the manuscript for more detailed information
# n and p: the size of the generated matrices
# rank: a length-two vector. The elements are the total rank for the first and second matrix respectively
# joint_rank_col: the specified joint column rank
# joint_rank_row: the specified joint row rank
# nrep: the number of replications. 
# noise: either TRUE or FALSE. When TRUE, we add noise.
# std1: the variance of gaussian distribution of noise matrix 1
# std2: the variance of gaussian distribution of noise matrix 2
# Note: if std_i = sqrt(r_i/(n*p)), the Frobenius norm of the signal to noise ratio is 1. 
DoubleDataGen2 <- function(n = 1000, p = 400, rank = c(130,150), joint_rank_col = 30, joint_rank_row = 50, nrep = 20, noise = TRUE, std1 = 0.1, std2 = 0.1){
  # Check if the inputs meet some requirements.
  if (n %% 4 != 0){stop("4 must be a factor of n.")}
  if (p %% 4 != 0){stop("4 must be a factor of p.")}
  if (max(rank) > min(n, p)){stop("Total rank exceeds the size of matrices.")}
  ...
  for (i in 1:nrep){
    ...
    # Vector of Singular values for matrix 1
    # Simply put it as 1.
    Dvec1 = rep(1,rank[1])
    # Normal case. Use diag(...) to make a diagonal matrix.
    if (rank[1] >= 2){
      Dvec1 = diag(sort(Dvec1, decreasing = TRUE))
    }
    # If rank = 1, just make it a matrix that only contains one element.
    if (rank[1] < 2){
      Dvec1 = as.matrix(Dvec1)
    }
    
    # Vector of Singular values for matrix 2
    # Simply put it as 1.
    Dvec2 = rep(1,rank[2])
    if (rank[2] >= 2){
      Dvec2 = diag(sort(Dvec2, decreasing = TRUE))
