...
# DMMD with all ranks being fixed
DMMD_Irina <- function(X1, X2, r1, r2, rc, rr, eps = .Machine$double.eps^0.5, kmax = 1000, verbose = FALSE){
  n = nrow(X1)
  p = ncol(X1)
  X1F2 = sum(X1^2)
  X2F2 = sum(X2^2)
...
# DMMD with unknown ranks
DMMD_Irina_unknown <- function(X1, X2, eps = .Machine$double.eps^0.5, kmax = 1000, verbose = FALSE){
  n = nrow(X1)
  p = ncol(X1)
