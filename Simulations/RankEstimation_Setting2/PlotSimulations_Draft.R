rm(list = ls())
load("Simulations/RankEstimation_Setting2/SLIDEoutput.RData")
output_slide = output
rm(output)
load("Simulations/RankEstimation_Setting2/AJIVEoutput.RData")
output_ajive = output
rm(output)
load("Simulations/RankEstimation_Setting2/output.RData")
...
# Make a table which shows the summary statistics of rank estimation results of different methods 

# There are 56 NAs in SLIDE result.
# is.na(my_error_joint_rank_col_PL)
# is.na(jive_error_joint_rank_c)
# is.na(ajive_error_joint_rank_c)
is.na(slide_error_joint_rank_c)
# is.na(my_error_joint_rank_row_PL)
# is.na(jive_error_joint_rank_r)
# is.na(ajive_error_joint_rank_r)
is.na(slide_error_joint_rank_r)

summary_table_joint = data.frame(my_error_joint_rank_col_PL, jive_error_joint_rank_c, 
                           ajive_error_joint_rank_c, slide_error_joint_rank_c,
                           my_error_joint_rank_row_PL, jive_error_joint_rank_r,
                           ajive_error_joint_rank_r, slide_error_joint_rank_r)
summary(summary_table_joint)
...
summary_table_total = data.frame(my_error_total_rank1_PL, my_error_total_rank1_ED, 
                                 jive_error_total_rank1_c, jive_error_total_rank1_r,
                                 slide_error_total_rank1_c,slide_error_total_rank1_r,
                                 my_error_total_rank2_PL, my_error_total_rank2_ED,
                                 jive_error_total_rank2_c, jive_error_total_rank2_r,
...
summary(summary_table_total)

# Draw boxplot
fig.path = "Simulations/RankEstimation_Setting2/Figures/Draft/"
pdf(file = paste(fig.path,"Comparison on Joint Rank Estimation_Setting2.pdf",sep=""), width = 10, height = 7)
print(gg_joint)
dev.off()

pdf(file = paste(fig.path,"Comparison on Total Rank Estimation_Setting2.pdf",sep=""), width = 10, height = 7)
print(gg_total)
dev.off()
